...
def replace_lora_with_bitlinear(model):
    replaced_count = 0
    for name, module in model.named_modules():
        # PEFT에서 LoRA 어댑터는 보통 'lora_A', 'lora_B'라는 이름의 ModuleDict 안에 들어있습니다.
        if ("lora_A" in name or "lora_B" in name) and isinstance(module, nn.ModuleDict):
            for adapter_name, linear_layer in module.items():
                if isinstance(linear_layer, nn.Linear):
                    # 1) 동일한 크기의 BitLinear 생성
                    ...
                    # 2) PEFT가 초기화해둔 가중치 그대로 복사
                    bit_linear.weight.data = linear_layer.weight.data.clone()
                    if linear_layer.bias is not None:
                        bit_linear.bias.data = linear_layer.bias.data.clone()
                    
                    # 3) 디바이스(GPU 등) 및 dtype 맞추기
                    bit_linear.to(linear_layer.weight.device, dtype=linear_layer.weight.dtype)
                    
                    # 4) 기존 레이어를 BitLinear로 완벽히 바꿔치기
                    module[adapter_name] = bit_linear
                    replaced_count += 1
    return replaced_count

count = replace_lora_with_bitlinear(model)
print(f">>> 총 {count}개의 LoRA 어댑터 행렬이 1.58-bit BitLinear로 교체되었습니다.")
...
training_args = TrainingArguments(
    output_dir="./medical_bitlora_results",
    # --- 메모리 방어 핵심 세팅 ---
...
# 5. 학습 완료 후 최종 어댑터 저장
save_path = "./medical_bitlora_adapter_final"
